...
main_source_files = ['duckdb_python.cpp'] + glob('src/**/*.cpp', recursive=True)
...
if len(existing_duckdb_dir) == 0:
    ...
    if os.path.isfile(os.path.join(script_path, '..', '..', 'scripts', 'amalgamation.py')):
        ...
        package_build = __import__('package_build')

        (source_list, include_list, original_sources) = package_build.build_package(os.path.join(script_path, lib_name),
                                                                                    extensions, False, unity_build)
        ...
        amalgamation = __import__('amalgamation')
