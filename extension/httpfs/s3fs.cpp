	...
	try {
		...
		if (res->code != 200) {
			throw HTTPException(res->code, res->error,
			                    "Unable to connect  to URL " + file_handle.path + " " + res->error + " (HTTP code " +
			                        to_string(res->code) + ")");
		}
...
void S3FileSystem::FinalizeMultipartUpload(S3FileHandle &file_handle) {
	...
	if (open_tag_pos == string::npos) {
		throw HTTPException(res->code, res->error, "Unexpected response during S3 multipart upload finalization");
	}
